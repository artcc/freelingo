...
- **POST `/from-word`** — Rate limit: 30/min. Saves a single word as a flashcard: body `{word, context, cefr_level}`. Deduplicates by normalized word (same normalization as `/bulk`) against the user's flashcards on the active plan: if the input word already exists, returns the existing card without calling the AI; otherwise AI generates definition/example/translation (which may canonicalize the word to a different lemma), and if that canonicalized word also already exists, returns the existing card instead of inserting. Only inserts (`source="from_text"`) when neither check matches. Returns `FlashcardFromWordResponse` (`FlashcardResponse` plus `already_saved: bool`, true when an existing card was returned instead of a new one being created)
