...
- LLM — Key variables: `LLM_PROVIDER`, `OLLAMA_*`, `OPENAI_*`, `ANTHROPIC_*`, `DEEPSEEK_*`; Notes: Provider selected via `LLM_PROVIDER`; `ANTHROPIC_MAX_TOKENS` defaults to 8192 and must not exceed the selected model's output limit; `LLM_REQUEST_TIMEOUT` (default 120) bounds each LLM request and should be raised for slow local models
