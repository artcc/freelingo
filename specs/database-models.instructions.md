...
- `translations` — required PostgreSQL JSONB object containing `title`, `subtitle`, and `description` for the fifteen UI locales: `en`, `es`, `fr`, `pt`, `de`, `it`, `ru`, `nl`, `pl`, `ro`, `tr`, `sv`, `da`, `fi`, and `hr`. New saves require all fifteen; previously stored ten- through fourteen-locale objects remain readable until missing translations are added by an administrator.
