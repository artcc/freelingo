...
const locales = ['en', 'es', 'fr', 'pt', 'de', 'it', 'ru', 'nl', 'pl', 'ro', 'tr', 'sv', 'da', 'fi']
...
describe('Admin system dashboard banner', () => {
  ...
  it('loads a legacy announcement and requires missing locales before saving it', async () => {
    const { tr, sv, da, fi, ...legacyTranslations } = generatedTranslations
    mockApiFetch.mockReset()
    mockApiFetch
      .mockResolvedValueOnce(
        new Response(
    ...
    expect(await screen.findByText('10/14 complete')).toBeVisible()
    ...
    expect(screen.getByText('11/14 complete')).toBeVisible()
    expect(screen.getByText('dashboardBanner.save')).toBeDisabled()
    fireEvent.change(screen.getByLabelText('dashboardBanner.editTranslation'), {
      target: { value: 'sv' },
    })
    ...
    expect(screen.getByText('12/14 complete')).toBeVisible()
    expect(screen.getByText('dashboardBanner.save')).toBeDisabled()
    fireEvent.change(screen.getByLabelText('dashboardBanner.editTranslation'), {
      target: { value: 'da' },
    })
    ...
    expect(screen.getByText('13/14 complete')).toBeVisible()
    expect(screen.getByText('dashboardBanner.save')).toBeDisabled()
    fireEvent.change(screen.getByLabelText('dashboardBanner.editTranslation'), {
      target: { value: 'fi' },
    })
    ...
    expect(screen.getByText('14/14 complete')).toBeVisible()
    fireEvent.click(screen.getByText('dashboardBanner.save'))
